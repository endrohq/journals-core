...
import { getTreasuryContextForEvent } from './treasury';
...
export const findEventById = async (id: string, dataAccess: BaseModuleDataAccess) => {
	const [events, treasuryContext] = await Promise.all([
		findEvents(dataAccess),
		getTreasuryContextForEvent(dataAccess, id),
	]);

	const event = events.find(item => item.id === id);
	if (!event) return undefined;
	event.treasury = {
		supporters: treasuryContext.supporters,
		funding: treasuryContext.funding,
	};
	const labels =
